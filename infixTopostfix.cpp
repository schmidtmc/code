...
int main()
{
	Queue_dynamic<char> infix, postfix;	// Dynamic Queue creation
	Stack_dynamic<char> opStack;			// Dynamic Stack creation. operator stack
	Stack_dynamic<char> evalStack;		// Stack to be evaluated.
	...
	if(!fin)							
	{
		cout << "Cannot open input file" << endl;	// Test to see if file exists
		exit(1);
	}
	...
	while(fin.good()) {
			...
			while (run == 1)     // loop while extraction from file is possible
			{
				...
				{
					if( c != ' ' && c != '\r' && c != '\t' && c != '\n') // if char is not a blank or carriage return
			...
			while(infix.isEmpty() == true)	// While infix is full execute
			{
				...
				if(c != '+' && c != '-' && c != '/' && c != '*' && c != '(' && c != ')') {
					...
					while(opStack.isEmpty() == true && runOp == 1) {
						opToken = opStack.pop();
