...
#include "Queue_dynamic.cpp" // Required to be included by compiler.
#include "Stack_dynamic.cpp" // Required to be included by compiler.
...
int main()
{
    Queue_dynamic<char> infix, almost, prefix;
    Stack_dynamic<int> intStack;
	Stack_dynamic<char> opStack, prepStack, charStack;
	...
	while(fin.good()) {
			...
			while (run == 1)     // loop while extraction from file is possible
			{
				...
				{
					if( c != ' ' && c != '\r' && c != '\n' && c != '\t') // if char is not a blank or carriage return
